...
char* tokenToCharacter(int token);
    ...
    //Main Function
    void scanner(int print)
	{
        //Open input and output files
        input =   fopen("scannerinput.txt", "r");
        output =   fopen("parsercodgeninput.txt", "w");

        //If the file isn't there or could not opened properly, end the program.
        if(input == NULL){
            printf("File could not open. Program halted.\n");
            return;
        }

        //Reads in the input file character by character.
        char temp;
        int i = 0;
	...
	{   symflag = 0;
		if (tokens[i]== numbersym)
		{
		    ...
		    if(symflag==0){
                fprintf(output, "%d %d ", tokens[i], sym_cnt);
                if(print)
		...
		{
		    strcpy(temp_id, id_list[id_cnt]);
		    // Checks if we already printed this out.
		    for(j=0; j<cnt; j++){
                if(strcmp(st[j].name, temp_id)==0){
                    fprintf(output, "%d %s ", tokens[i], temp_id);
                    if(print)
